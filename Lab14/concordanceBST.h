...
template <typename DataType>
inline void BST<DataType>::insert(DataType & item)
{
	...
	{
		locptr->data.a.enqueue(item.lineNum);
	}
}
...
//assignment operator
template <typename DataType>
BST<DataType>& BST<DataType>::operator=(const BST<DataType>& origRoot)
{
	if (this != &origRoot)
	{
		destroy(myRoot);
		copyTree(origRoot.myRoot, myRoot);
	}
	return *this;
}
